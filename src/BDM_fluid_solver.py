...
from ufl       import inner, dot, div, jump, avg, nabla_grad
...
class FluidSolverALE:
    ...
    def setup(self):
        ...
        if self.solver_type=="navier-stokes":
            ...
            a00 += rho*inner(dot(c_vel, nabla_grad(u)), v)*dx # Convective term
    ...
    def solve(self):
        ...
        if self.calc_cilia_direction_vectors:
            ...
            for i, t in enumerate(self.times):
                ...
                self.ph_.x.array[:] += (self.mean_ICP - mean_pressure_)
