...
\begin{equation}
\rho\frac{\mathrm D\uu}{\mathrm Dt}  = \rho\left(\pdifft{\uu} + (\uu\cdot\nabla)\uu\right) = \nabla\cdot\TT
...
approximation, and previous studies have modeled 
...
backward Euler scheme. Since we use an implicit
time discretization, we only need an initial condition
...
Convergence properties of the fluid discretization scheme was assessed with two
...
In terms of physiological validity, the pressure gradients
and flow rates are compareable to results in previous works.
\begin{figure}
    \centering
...
We present FTLE fields for the different brains in. The repelling
and attracting structures in the anterior-middle interventricular
