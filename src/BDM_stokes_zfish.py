...
def setup_stokes_problem(mesh: dfx.mesh.Mesh, ft: dfx.mesh.MeshTags):
    ...
    bdm1_el = element("BDM", mesh.basix_cell(), degree=1) # Brezzi-Douglas-Marini elements
    dg0_el  = element("DG", mesh.basix_cell(), degree=0) # Discontinuous Galerkin elements
    V = dfx.fem.functionspace(mesh, bdm1_el) # Velocity function space
    Q = dfx.fem.functionspace(mesh, dg0_el) # Pressure function space
    ...
    # Impose impermeability condition on boundary except spinal canal out/lateral apertures
    v_zero = dfx.fem.Function(V) # Zero velocity
    imperm_facets = np.concatenate(([ft.find(tag) for tag in impermeability_tags]))
    v_dofs_imperm = dfx.fem.locate_dofs_topological(V, facet_dim, imperm_facets)
...
if __name__=="__main__":
    ...
    velocity_output_filename = f"../output/zfish-mesh/flow/BDM_velocity.bp"
    velocity_output = dfx.io.VTXWriter(comm, velocity_output_filename, [uh_out], "BP4")
    pressure_output_filename = f"../output/zfish-mesh/flow/BDM_pressure.bp"
    pressure_output = dfx.io.VTXWriter(comm, pressure_output_filename, [ph], "BP4")

    if write_cpoint:
        cpoint_filename = f"../output/zfish-mesh/flow/checkpoints/BDM_velocity"
