...
class FluidSolverALE:
    ...
    def setup(self):
        ...
        if self.write_output:
            velocity_output_filename = f"../output/{self.mesh_prefix}-mesh/flow/{self.solver_type}/BDM_deformation_velocity_T={self.T}_dt={self.timestep}.pvd"
            self.velocity_output = dfx.io.VTKFile(self.comm, velocity_output_filename, "w")
            self.velocity_xdmf = dfx.io.XDMFFile(self.comm, velocity_output_filename.removesuffix(".pvd") + ".xdmf", "w")
            ...
            pressure_output_filename = f"../output/{self.mesh_prefix}-mesh/flow/{self.solver_type}/BDM_deformation_pressure_T={self.T}_dt={self.timestep}.pvd"
            self.pressure_output = dfx.io.VTKFile(self.comm, pressure_output_filename, "w")

        if self.write_cpoint:
            self.cpoint_filename = f"../output/{self.mesh_prefix}-mesh/flow/{self.solver_type}/checkpoints/BDM_deformation_velocity_T={self.T}_dt={self.timestep}"
