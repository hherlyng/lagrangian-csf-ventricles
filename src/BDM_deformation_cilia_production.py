...
class FluidSolverALE:
    ...
    def setup(self):
        ...
        if self.write_output:
            velocity_output_filename = f"../output/{self.mesh_prefix}-mesh/flow/{self.solver_type}/BDM_deformation+cilia+production_velocity_T={self.T:.4g}_dt={self.timestep:.4g}.pvd"
            ...
            pressure_output_filename = f"../output/{self.mesh_prefix}-mesh/flow/{self.solver_type}/BDM_deformation+cilia+production_pressure_T={self.T:.4g}_dt={self.timestep:.4g}.pvd"
            self.pressure_output = dfx.io.VTKFile(self.comm, pressure_output_filename, "w")

        if self.write_cpoint:
            self.cpoint_filename = f"../output/{self.mesh_prefix}-mesh/flow/{self.solver_type}/checkpoints/BDM_deformation+cilia+production_velocity_T={self.T}_dt={self.timestep}"
