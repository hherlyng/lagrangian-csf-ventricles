...
f = rho*12*theta*tt**2*theta_vector - div(sigma(u_exact))
# f = - div(sigma(u_exact))


# The weak form
a = rho*inner(w/(beta*dt**2), dw)*dx + inner(sigma(w), eps(dw)) * dx
L = rho*inner(wh_n/(beta*dt**2) + wh_dot_n/(beta*dt) + (1-2*beta)/(2*beta)*wh_ddot_n, dw)*dx + inner(f, dw) * dx
...
xdmf = dfx.io.XDMFFile(comm, f"../output/square-mesh/deformation/time_dep_displacement_dt={timestep:.4g}_T={T:.4g}.xdmf", "w")
xdmf_vel = dfx.io.XDMFFile(comm, f"../output/square-mesh/deformation/time_dep_deformation_velocity_dt={timestep:.4g}_T={T:.4g}.xdmf", "w")
...
vh_cpoint_filename = f"../output/square-mesh/deformation/checkpoints/time_dep_deformation_velocity_dt={timestep:.4g}_T={T:.4g}/"
