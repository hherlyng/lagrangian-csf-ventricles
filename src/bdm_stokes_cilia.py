...
impermeability_tags = cilia_tags
...
def setup_stokes_problem(mesh: dfx.mesh.Mesh, ft: dfx.mesh.MeshTags, mesh_prefix: str):
    ...
    facets_imperm = np.concatenate(([ft.find(tag) for tag in impermeability_tags]))
    v_dofs_imperm = dfx.fem.locate_dofs_topological(V, facet_dim, facets_imperm)
...
if __name__=='__main__':
    ...
    a, L, bcs, V, Q, ds = setup_stokes_problem(mesh, ft, mesh_prefix)
    ...
    velocity_output_filename = f"../output/{mesh_prefix}-mesh/flow/velocity_cilia.pvd"
    velocity_output = dfx.io.VTKFile(comm, velocity_output_filename, "w")
    pressure_output_filename = f"../output/{mesh_prefix}-mesh/flow/pressure_cilia.pvd"
    pressure_output = dfx.io.VTKFile(comm, pressure_output_filename, "w")

    if write_cpoint:
        cpoint_filename = f"../output/{mesh_prefix}-mesh/flow/checkpoints/velocity_cilia"
