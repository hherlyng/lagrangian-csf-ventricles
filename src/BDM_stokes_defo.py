...
def setup_stokes_problem(mesh: dfx.mesh.Mesh, ft: dfx.mesh.MeshTags):
    ...
    # Impose deformation velocity on the boundary except at spinal canal out/lateral apertures
    v_defo = dfx.fem.Function(V) # Deformation velocity
    facets_defo = np.concatenate(([ft.find(tag) for tag in displacement_tags]))
    v_dofs_defo = dfx.fem.locate_dofs_topological(V, facet_dim, facets_defo)

    bcs = [dfx.fem.dirichletbc(v_defo, v_dofs_defo)]

    return a, L, bcs, V, Q, ds, v_defo
...
if __name__=='__main__':
    ...
    velocity_output_filename = f"../output/{mesh_prefix}-mesh/flow/velocity_defo.pvd"
    velocity_output = dfx.io.VTKFile(comm, velocity_output_filename, "w")
    pressure_output_filename = f"../output/{mesh_prefix}-mesh/flow/pressure_defo.pvd"
    pressure_output = dfx.io.VTKFile(comm, pressure_output_filename, "w")

    if write_cpoint:
        cpoint_filename = f"../output/{mesh_prefix}-mesh/flow/checkpoints/velocity_defo"
